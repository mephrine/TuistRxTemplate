...
def clear_image_asset_dict(tree):
    ...
    for key in list(tree.keys()):
        if ".imageset" in key:
            ...
            clear_image_asset_dict(tree[key])
    ...
    for key in keyList:
        if ".imageset" in key:
            ...
            for i in range(level):
                f.write(" ")
            f.write("public static var " + assetName +
                    ": UIImage { .R(#imageLiteral (resourceName: \"" + imageName + "\")) }\n")
        else:
            for i in range(level):
                f.write(" ")
